...
export interface GOOGLE_MAP_SEARCH_RESULT {
	...
	result: {
		...
		opening_hours: {
			open_now: boolean,
			periods: { close: { day: number, time: number}, open: { day: number, time: number} }[],
			weekday_text: string[],
		},
		photos: { height: number, html_attributions: string[], photo_reference: string, width: number }[],
		place_id: string,
		rating: number,
		reviews: { author_name: string, author_url: string, language: string, profile_photo_url: string, rating: number,
			relative_time_description: string, text: string, time: number }[],
		types: string[],
	};
	status: string;
}
