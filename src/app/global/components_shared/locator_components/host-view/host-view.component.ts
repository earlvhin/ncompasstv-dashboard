...
export class HostViewComponent implements OnInit, OnDestroy {
        ...
        this._host
            ...
            .subscribe(
                (response: { category: any[]; paging: PAGING; message?: string }) => {
                    ...
                    this.categoriesData = [...(response.paging.entities as API_CATEGORY[])].filter(
                        (category) => category.totalLicenses > 0,
                    );
                },
                (error) => {
                    console.error(error);
    ...
    private getHosts(page: number): void {
        ...
        const getDealerHostsRequest = this._host.get_host_by_dealer_id_locator(
            this.searchDealerId,
            page,
            search,
            0,
