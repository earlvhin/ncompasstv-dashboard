...
export class HostViewComponent implements OnInit, OnDestroy {
	...
	constructor(private _auth: AuthService, private _host: HostService, private _license: LicenseService, private _router: Router) {}
				...
				(response: { host: API_HOST[]; paging: PAGING; distinctStates: API_HOST[]; message?: string }) => {
					...
					if (this.currentSearchOption === 'state' || this.currentSearchOption === 'category') {
						...
						if (this.search_state_key) {
							...
							this.host_results.forEach((x) => {
								x.icon_url = this.map_markers.filter((y) => y.hostId === x.hostId)[0].icon_url;
							});
		...
		this._host
			...
			.subscribe(
				(response: { dealer: API_DEALER; host: API_HOST }) => {
					...
					this.selected_host.latitude
						? (this.selected_host.latitude = parseFloat(this.selected_host.latitude).toFixed(5))
						: (this.selected_host.latitude = '-');
					this.selected_host.longitude
						? (this.selected_host.longitude = parseFloat(this.selected_host.longitude).toFixed(5))
						: (this.selected_host.longitude = '-');
	...
	getHostStates(page: number) {
		...
		this._host
			.get_host_states(page, this.search_state_key, this.searchDealerId)
			.pipe(takeUntil(this._unsubscribe))
			.subscribe(
	...
	getHostCategories(page: number) {
		...
		this._host
			.get_host_categories(page, this.search_category_key, this.searchDealerId)
			.pipe(takeUntil(this._unsubscribe))
			.subscribe(
