...
export class LicenseViewComponent implements OnInit {
	...
	constructor(private _auth: AuthService, private _host: HostService, private _license: LicenseService) {}
	...
	exportToCSV(): void {
		...
		this.map_markers.forEach(
			license => {
				...
				let marker = new UI_DEALER_LOCATOR_EXPORT(businessName, license.name, locatorAddress, license.category, this.markStoreHours, license.latitude, license.longitude);
				this.exported_map_marker.push(marker);
			}
		);

		const header = Object.keys(this.exported_map_marker[0]);
		const csv = this.exported_map_marker.map((row) => header.map((fieldName) => JSON.stringify(row[fieldName], replacer)).join(','));
	...
	filterDealerHosts(value: number): void {
		...
		this.selected_dealer_hosts = this.selected_dealer_hosts.filter(host => this.selected_host_ids.includes(host.hostId));
		this.selected_dealer_hosts.forEach(host => this.getLicenseByHost(host.hostId));
	...
	searchData(key: string): void {
		...
		this._license.search_license(key).pipe(takeUntil(this._unsubscribe))
			.subscribe(
				response => {
					...
					dealerLicenses.map(
						license => {
							...
							let dealerLicense = new API_DEALER_LICENSE(
								...
								license.licenseId, license.licenseKey
							);
			...
			.subscribe(
				response => {
					...
					if (this.license_search_results !== undefined) {
						this.host_licenses = this.host_licenses.filter((license) => license.licenseId === this.search_license_id);
					}
