...
export class LicenseModalComponent implements OnInit {
	...
	ngOnInit() { 
		...
		this.subscription.add(
			this.generate_license_form.valueChanges.subscribe(
				data => {
					if (this.generate_license_form.valid && this.f.number_of_license.value > 0 && this.f.number_of_license.value <= 50) {
	...
	get f() { return this.generate_license_form.controls; }
	...
	getDealers(e) {
		if(e > 1) {
			...
			console.log("IS SEARCH", this.is_search)
			if(this.is_search) {
				this.loading_search = true;
			}
			this.subscription.add(
