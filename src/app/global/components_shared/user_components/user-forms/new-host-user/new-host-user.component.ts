	...
	getHostPlaces(page) {
		if (this._auth.current_user_value.role_id === UI_ROLE_DEFINITION.dealer) {
			...
			if (page > 1) {
				...
				this.subscription.add(
					this._host.get_host_by_dealer_id(this._auth.current_user_value.roleInfo.dealerId, page, this.search_key).subscribe((data) => {
			...
			if (page > 1) {
				this.loading_data = true;
				this.subscription.add(
					this._host.get_host_by_page({ page, search: this.search_key, sortColumn: '', sortOrder: '', pageSize: 15 }).subscribe((data) => {
						data.paging.entities.map((i) => {
							if (this.search_key != '') {
								this.hosts.push(i);
							} else {
