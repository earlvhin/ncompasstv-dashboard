...
export class NewDealerAdminComponent implements OnInit {
    ...
    getDealers() {
        this.subscription.add(
			this._dealer.get_dealers_with_page(1, '', 0).subscribe(
				data => {
	...
	createNewAdmin(formDirective) {
        ...
        this.new_admin_form.value.dealers = this.new_admin_form.value.dealers.map(
            dealer => {
		...
		this.subscription.add(
			this._user.create_new_user(this.f.roleid.value, this.new_admin_form.value).subscribe(
				data => {
					this.is_submitted = false;
					this.form_invalid = false;
					this.openConfirmationModal('success', 'Account creation successful!', 'Dealer Admin account has been added to database.');
				}, 
				error => {
					this.is_submitted = false; 
					this.form_invalid = false;
