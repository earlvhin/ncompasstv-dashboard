...
export class ScreenLicenseComponent implements OnInit {
	...
	assignLicenses() {
		...
		if (this.assigning_license) {
			...
			this._screen.assign_license(licenseScreens).pipe(takeUntil(this._unsubscribe))
				.subscribe(
					() => {
	...
	blacklistAddedContents() {
		...
		this._playlist.blocklist_content(this.to_blacklist).pipe(takeUntil(this._unsubscribe))
			.subscribe(
				data => {
					this.assign_success = true;
					this.assigning_license = false;
	...
	private getLicenses(): void {
		...
		this._license.get_licenses_by_host_id(this.licenseId).pipe(takeUntil(this._unsubscribe))
			.subscribe(
				response => {
				...
				error => console.log('Error retrieving host licenses', error)
			)
