...
export class ChangeTemplateComponent implements OnInit, OnDestroy {
	...
	onSubmit() {
		...
		const { template, zones } = this.changeTemplateForm.value as { template: { id: string, name: string }, zones: { templateZoneId: string, playlistId: string }[]};
		...
		this.openConfirmDialog().pipe(takeUntil(this._unsubscribe))
			.subscribe(
				(response: boolean) => {
					if (!response) return;
