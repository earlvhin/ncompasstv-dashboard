...
export class CloneScreenComponent implements OnInit {
    ...
    async cloneScreen(): Promise<void> {
        ...
        try {
            const cloneResponse = await this._screen
                .create_screen(screen)
                .pipe(takeUntil(this._unsubscribe))
                .toPromise();
            this.cloned_screen_id = cloneResponse.screenId;
            const clonedScreenData = await this._screen
                .get_screen_by_id(this.cloned_screen_id)
                .pipe(takeUntil(this._unsubscribe))
                .toPromise();
    ...
    private getHostByDealer(page: number): void {
        ...
        if (page > 1) {
            ...
            this._host
                .get_host_by_dealer_id(
                    this.selected_dealer[0].dealerId || this.selected_dealer,
                    page,
                    this.searchHostKeyword,
                )
                .pipe(takeUntil(this._unsubscribe))
