...
export class PlaylistEditModalComponent implements OnInit {
	...
	ngOnInit() {
		//console.log('#playlistEditModal', this._playlist_data);
		this.edit_form = this._form.group(
			{
				...
				playlistDescription: [this._playlist_data.playlist.playlistDescription, [Validators.required, Validators.maxLength(100)]]
			}
		)

		this.subscription.add(
			this.edit_form.valueChanges.subscribe(
				(data: any) => {
	...
	get f() { return this.edit_form.controls; }
