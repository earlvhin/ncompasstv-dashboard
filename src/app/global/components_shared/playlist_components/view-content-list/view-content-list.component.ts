...
export class ViewContentListComponent implements OnInit, OnDestroy {
	...
	table_data: { columns: { name: string }[], rows: { title: string, type: string, duration: number, isFullScreen: number, url: string }[] };
	title = 'Playlist Content List';
	
	constructor(
		@Inject(MAT_DIALOG_DATA) public _dialog: { contents: API_CONTENT[] },
	) { }
	...
	private mapToTable(): { title: string, type: string, duration: number, isFullScreen: number, url: string }[] {
