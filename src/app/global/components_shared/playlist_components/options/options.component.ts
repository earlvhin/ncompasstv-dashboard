...
export class OptionsComponent implements OnInit, OnDestroy, AfterContentChecked {
    ...
    playlist_changes_data: PLAYLIST_CHANGES = {
        content: null,
        blocklist: null,
        credits: null,
        credits_status: null,
    ...
    contentDataChanged() {
        if (
            JSON.stringify(this.content_data) === localStorage.getItem('playlist_data') &&
            this.blocklist_changes.status == false
        ) {
            this.unchanged_playlist = true;
        } else {
    ...
    onToggleCredits(): void {
        ...
        if (enabled !== initialStatus)
            this.playlist_changes_data.credits_status = {
                playlistContentId,
                licenseId,
                status: enabled,
            };
        if (enabled === initialStatus && 'credits_status' in this.playlist_changes_data)
            delete this.playlist_changes_data.credits_status;
        this.contentDataChanged();
    }
    ...
    saveBlocklistChanges(data: {
        incoming: API_BLOCKLIST_CONTENT[];
        removing: API_BLOCKLIST_CONTENT[];
        status: boolean;
    }) {
