...
export class ScreenshotPipe extends DatePipe implements PipeTransform {
	...
	transform(value: any, ...args: any[]): any {
		...
		let s_time = handle_removed.substring(handle_removed.lastIndexOf('--') + 2);
		s_time = s_time.substring(0, s_time.lastIndexOf('_'));
		s_time = s_time.split('_').join(':');
		let valid_time = s_time.toString().match(/^([01]\d|2[0-3])(:)([0-5]\d)(:[0-5]\d)?$/) || [s_time];
	  
		if (valid_time.length > 1) { // If valid_time format correct
