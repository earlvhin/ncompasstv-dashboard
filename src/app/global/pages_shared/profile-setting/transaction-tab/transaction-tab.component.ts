...
export class TransactionTabComponent implements OnInit {
    ...
    getTransactions(page) {
        ...
        this.subscription.add(
            this._billing.get_transaction_charges(page, 15, this.dealerId, this.end_date, this.start_date, this.filtered_status, this.filtered_type).pipe(takeUntil(this._unsubscribe)).subscribe(
                (response:any) => {
                    if(!response.message) {
		...
		return data.map(
			h => {
				const table = new UI_DEALER_TRANSACTIONS (
					...
					{ value: moment(h.date).format("MMM DD YYYY"), link: null , editable: false, hidden: false},
					{ value: h.orderNo , link: null, editable: false, hidden: false},
					{ value: h.description.split(" ")[0] === 'Purchase' ? 'Purchase' : 'Subscription', link: null, editable: false, hidden: false},
					{ value: h.description, link: null, editable: false, hidden: false},
					{ value: h.amount > 0 ? '$ ' + h.amount: '$ ' + 0, link: null, editable: false, hidden: false },
