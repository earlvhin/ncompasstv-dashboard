...
export class SingleScreenComponent implements OnInit {
    ...
    onAssignLicense() {
        const dialog = this._dialog.open(ScreenLicenseComponent, {
            disableClose: true,
            data: {
                license_id: this.screen.assigned_host_id,
                screen_id: this.screen.screen_id,
                zone_contents: this.screen.screen_zone_playlist,
            },
        });
        ...
        const config: MatDialogConfig = {
            ...
            data: {
                ...
                templates: this.templates.filter(
                    (data) => data.template.templateId !== this.currentTemplate.template.templateId,
                ),
            },
        };
    ...
    private mapToScreenLicenseUI(data: any) {
        ...
        return data.map((l: API_LICENSE_PROPS) => {
            return new UI_SCREEN_LICENSE_SCREENS(
                ...
                {
                    value: l.internetType ? this.getInternetType(l.internetType) : '--',
                    link: null,
                    editable: false,
                    hidden: false,
                },
                {
                    value: l.internetSpeed ? l.internetSpeed : '--',
                    link: null,
                    editable: false,
                    hidden: false,
                },
    ...
    private setPageData(data: API_SINGLE_SCREEN) {
        ...
        this.getTemplates().add(
            () =>
                (this.currentTemplate = this.templates.filter(
                    (data) => data.template.templateId === this.screen.assigned_template_id,
                )[0]),
        );
        ...
        this.screen_info = this._form.group({
            ...
            type: [
                {
                    value: this.screen.type ? this.screen.type : null,
