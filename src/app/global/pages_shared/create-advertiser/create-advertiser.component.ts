...
export class CreateAdvertiserComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.subscription.add(
			this.google_place_form.valueChanges.subscribe((data) => {
	...
	googleMap() {
		...
		this.subscription.add(
			this._map.get_google_location_info(this.g.location.value).subscribe((data: API_GOOGLE_MAP['google_search']) => {
				if (data.length > 0) {
	...
	watchCategoryField() {
		this.f.category.valueChanges.subscribe((data) => {
			if (data === '') {
