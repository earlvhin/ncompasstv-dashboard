...
export class OrdersComponent implements OnInit {
	...
	constructor(private _billing: BillingService, private _auth: AuthService, private _dialog: MatDialog) {}
	...
	getOrders(page) {
		...
		if (this.dealerId == '') {
			this.subscription.add(
				this._billing
					.get_billing_purchases(page, 15, this.search_data_orders, this.end_date, this.start_date, this.filtered_status)
			...
			this.subscription.add(
				this._billing
					.get_billing_purchases_per_dealer(this.dealerId, page, 15, this.end_date, this.start_date, this.filtered_status)
					.pipe(takeUntil(this._unsubscribe))
					.subscribe((response) => {
						if (!response.message) {
							this.orders_paging = response.paging;
