...
export class AddTagModalComponent implements OnInit, OnDestroy {
	...
	async onAddTag() {
		...
		try {
			...
			if (tagExists) {
				...
				const config = {
					width: '500px',
					height: '350px',
					data: { status: 'error', message: 'Cannot Add Tag', data: 'Tag already exists. Please assign it instead.' }
				};
		...
		newTagsControl.push({ name, tagColor, description, exclude, isVerifyingExistence, isExisting });
	...
	onSubmit() {
		const tagsToAdd = (this._newTagsControl.value as TAG[]).map((tag) => {
			return { name: tag.name, description: tag.description, tagColor: tag.tagColor, exclude: tag.exclude ? 1 : 0 };
		});

		const tagsToAssign = (this._existingTagsControl.value as TAG[]).concat(this.currentTags).map((tag) => tag.tagId);
		...
		this._tag
			...
			.subscribe(
				({ tags, message }) => {
					...
					const removedDuplicateTags = tags.filter((result) => !tagsToRemove.find((tagToRemove) => tagToRemove.tagId === result.tagId));
		...
		this._tag
			...
			.subscribe(
				({ tags, message }) => {
					...
					const filtered = searchResults.filter((result) => !tagsToRemove.find((tagToRemove) => tagToRemove.tagId === result.tagId));
