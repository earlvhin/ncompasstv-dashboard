...
export class DealerService extends BaseService {
    ...
    get_dealers_directory(page: number, key: string, searchKey: string) {
        return this.getRequest(
            `${this.getters.api_get_dealers_directory}` +
                '?page=' +
                `${page}` +
                '&search=' +
                `${key}` +
    ...
    get_dealers_with_license(
        page: number,
        key: string,
    ): Observable<{ dealers?: API_DEALER[]; paging?: PAGING; message?: string }> {
        const params = this.setUrlParams({ page, search: key });
        return this.getRequest(`${this.getters.api_get_dealers_with_license}${params}`);
    }

    get_dealers_with_page(
        page: number,
        key: string,
        pageSize = 15,
    ): Observable<{ dealers: API_DEALER[]; paging: PAGING }> {
    ...
    save_credit_card_details(
        data: UI_CREDIT_CARD_DETAILS,
        type = 'create',
    ): Observable<{ card: API_CREDIT_CARD_DETAILS }> {
