...
export class LicenseService extends BaseService {
	...
	get_all_licenses(
        ...
        const params = this.setUrlParams({ page, search: key , sortColumn: column, sortOrder: order, pageSize, includeAdmin: adminLicenses, piStatus: status, daysOffline: daysOffline, active: activated, daysInstalled: recent, timezone: zone, dealerId: dealer, hostId: host, assigned, pending, online, isActivated }, false, true);
		const url = `${base}${params}`;
		return this.getRequest(url);
	}
	...
	get_all_licenses_duration_clone(
        ...
        const params = this.setUrlParams({ page, search: key , sortColumn: column, sortOrder: order, pageSize, includeAdmin: adminLicenses, piStatus: status, daysOffline: daysOffline, active: activated, daysInstalled: recent, timezone: zone, dealerId: dealer, hostId: host }, false, true);
		const url = `${base}${params}`;
		return this.getRequest(url);
	}
	...
	sort_license_by_dealer_id(
        ...
        const base = `${this.getters.api_get_licenses_by_dealer}`;
        const params = this.setUrlParams({ dealerId: id, page, search: key , sortColumn: column, sortOrder: order, pageSize, piStatus: status, daysOffline, active: activated, daysInstalled: recent, timezone: zone,  hostId: host, assigned, pending, online, isActivated }, false, true);
		const url = `${base}${params}`;
		return this.getRequest(url);
	}
	...
	get_license_to_export_duration(
        ...
        const params = this.setUrlParams({ dealerId: id, search: key , sortColumn: column, sortOrder: order, pageSize, piStatus: status, daysOffline, active: activated, daysInstalled: recent, timezone: zone,  hostId: host, assigned, pending, online, isActivated }, false, true);
		const url = `${base}${params}`;
		return this.getRequest(url);
	}
	...
	get_license_resource_logs(license: string, date: string) {
        return this.getRequest(`${environment.base_uri}${environment.getters.api_get_resource_logs_by_date}${license}&selectedDate=${date}&page=1&pageSize=30`,null);	
	}
