...
export class LicenseService extends BaseService {
    ...
    httpParams = (params: object) =>
        new HttpParams({ encoder: new CustomHttpParamEncoder(), fromObject: { ...params } });
    ...
    get_outdated_licenses(filters: API_FILTERS): Observable<{
        appUiVersion?: string;
    ...
    get_license_resource_logs(licenseId: string, date: string) {
        return this.getRequest(
            `${this.getters.api_get_resource_logs_by_date}${licenseId}&selectedDate=${date}&page=1&pageSize=30`,
            null,
        );
    }
