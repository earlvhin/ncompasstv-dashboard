...
export class ScreensComponent implements OnInit {
	...
	getTotalScreens(id: string): void {
        this.screen_details = {};
		this._screen.get_screen_total_by_dealer(id).pipe(takeUntil(this._unsubscribe))
			.subscribe(
				(response: any) => {
					this.screen_details = {
				...
				error => console.log('Error retrieving dealer screen total', error)
			);
	...
	getAllScreens(page: number): void {
		...
		this._screen.api_get_screen_by_dealer_table(page, this.currentUser.roleInfo.dealerId, this.search_data)
			...
			.subscribe(
				...
				error => console.log('Error retrieving screen by dealer table ', error)
			)
	}
		...
		return data.map(
			screen => {
				return new UI_DEALER_TABLE_SCREEN (
					...
					{ value: this._titlecase.transform(screen.screenName), link: '/sub-dealer/screens/' +  screen.screenId, editable: false, hidden: false},
					{ value: screen.screenTypeName ? this._titlecase.transform(screen.screenTypeName) : '--', link: '/sub-dealer/hosts/' +  screen.hostId, editable: false, hidden: false},
