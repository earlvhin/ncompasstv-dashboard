...
export class HostsComponent implements OnInit {
	...
	getAdvertiserByDealer(page: number) {
		...
		this._advertiser.get_advertisers_by_dealer_id(this.currentUser.roleInfo.dealerId, page, this.keyword)
	...
	getHosts(page: number) {
		...
		this._host.get_host_by_dealer_id(this.currentUser.roleInfo.dealerId, page, this.search_data)
			.pipe(takeUntil(this._unsubscribe))
			.subscribe(
				response => {
	...
	private getDataForExport(): void {
        ...
        this._host.get_host_by_dealer_id(this.currentUser.roleInfo.dealerId, 1, '', this.pageSize)
			.pipe(takeUntil(this._unsubscribe))
			.subscribe(
				data => {
					if(!data.message) {
		...
		return data.map(
			hosts => {
				return new UI_DEALER_HOSTS(
					...
					{ value: hosts.name, link: `/${this.currentRole}/hosts/` + hosts.hostId, editable: false, hidden: false},
