...
export class DmaViewComponent implements OnInit, OnDestroy {
    ...
    getDMA(page, pageSize?): void {
        ...
        this._host.get_all_dma(page, this.search_key, pageSize).pipe(takeUntil(this._unsubscribe)).subscribe(
            data => {
    ...
    exportTable() {
        ...
        Object.keys(this.dma_table_column).forEach(key => {
            if(this.dma_table_column[key].name && !this.dma_table_column[key].no_export) {
                header.push({ header: this.dma_table_column[key].name, key: this.dma_table_column[key].key, width: 30, style: { font: { name: 'Arial', bold: true}}});
            }
        });
