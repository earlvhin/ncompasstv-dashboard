...
export class TagsTabComponent implements OnInit, OnDestroy {
	...
	constructor(private _auth: AuthService, private _date: DatePipe, private _license: LicenseService, private _title: TitleCasePipe) {}
	...
	private mapToTableFormat(licenses: API_LICENSE['license'][], paging: PAGING): UI_LICENSE_BY_TAGS[] {
		...
		return licenses.map((data) => {
			return {
				...
				screenshot: {
					value: data.screenshotUrl ? `${environment.base_uri}${data.screenshotUrl.replace('/API/', '')}` : null,
					link: data.screenshotUrl ? `${environment.base_uri}${data.screenshotUrl.replace('/API/', '')}` : null,
					editable: false,
					hidden: false,
					isImage: true,
					new_tab_link: true
