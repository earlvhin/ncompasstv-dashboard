	...
	private searchLicenses(filters: API_FILTERS) {
		...
		this._license.get_by_tags(filters, true).pipe(takeUntil(this._unsubscribe))
			.subscribe(
				...
				error => console.log('Error searching for licenses', error)
			);
	}
		...
		return licenses.map(
			data => {
				return {
					...
					id: { value: data.licenseId, link: null , editable: false, hidden: true, key: false, table: 'license'},
					screenshot: { 
						value: data.screenshotUrl ? `${environment.base_uri_old}${data.screenshotUrl.replace('/API/', '')}` : null,
						link: data.screenshotUrl ? `${environment.base_uri_old}${data.screenshotUrl.replace('/API/', '')}` : null, 
						editable: false, 
						hidden: false, 
						isImage: true
					},
					key: { value: data.licenseKey, link: `/${this.currentRole}/licenses/${data.licenseId}`, new_tab_link: 'true', compressed: true, editable: false, hidden: false, status: true },
					screenType: { value: data.screenType ? this._title.transform(data.screenType) : '--', editable: false, hidden: false },
					hostId: { value: data.hostId ? data.hostName : '--', link: data.hostId ? `/${this.currentRole}/hosts/${data.hostId}` : null, new_tab_link: 'true', editable: false, hidden: false },
					alias: { value: data.alias ? data.alias : '--', link: `/${this.currentRole}/licenses/${data.licenseId}`, editable: false, new_tab_link: 'true', label: 'License Alias', id: data.licenseId, hidden: false },
					tags: { value: data.tags, editable: false, label: 'Tags', type: 'tags', hidden: false },
