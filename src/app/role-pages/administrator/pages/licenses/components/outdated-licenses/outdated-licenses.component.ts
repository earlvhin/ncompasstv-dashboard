...
export class OutdatedLicensesComponent implements OnInit, OnDestroy {
	...
	constructor(private _auth: AuthService, private _datePipe: DatePipe, private _export: ExportService, private _license: LicenseService) {}
	...
	getDataForExport() {
		...
		this._license
			...
			.subscribe(async (response) => {
				let requests: Observable<{ appUiVersion?: string; appServerVersion?: string; paging?: PAGING; message?: string }>[] = [];
				...
				for (let i = NEXT_PAGE; i <= TOTAL_PAGES; i++) {
					requests.push(this._license.get_outdated_licenses({ pageSize: DEFAULT_PAGE_SIZE, page: i }));
				}
	...
	protected get _tableColumns() {
		return [
			...
			{ name: 'Status', sortable: false, key: 'pi_status', hidden: true, new_status: true, no_show: true },
			{ name: 'Alias', key: 'alias' },
