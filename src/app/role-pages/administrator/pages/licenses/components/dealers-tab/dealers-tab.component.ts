...
export class DealersTabComponent implements OnInit, OnDestroy {
    ...
    private getDealers() {
        return this._dealer
            .get_dealers_with_license(this.filters.page, this.filters.search)
        ...
        return data
            ...
            .map((dealer: API_DEALER) => {
                return new UI_TABLE_LICENSE_BY_DEALER(
                    ...
                    {
                        value: this._titlePipe.transform(dealer.contactPerson),
                        link: null,
                        editable: false,
                        hidden: false,
                    },
                    ...
                    },
                    {
                        value:
                            dealer.licenses.length > 0
                                ? this._datePipe.transform(dealer.licenses[0].dateCreated)
                                : '--',
                        link: null,
                        editable: false,
                        hidden: false,
                        ...
                        value:
                            dealer.licenses.length > 0
                                ? dealer.licenses.filter(
                                      (i) =>
                                          this._datePipe.transform(i.dateCreated) ==
                                          this._datePipe.transform(dealer.licenses[0].dateCreated),
                                  ).length
                                : 0,
                        link: null,
