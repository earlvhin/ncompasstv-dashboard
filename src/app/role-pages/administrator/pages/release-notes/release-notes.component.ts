...
export class ReleaseNotesComponent implements OnInit, OnDestroy {
	...
	private mapNotesToTable(data: API_RELEASE_NOTE[]): UI_RELEASE_NOTE[] {
		...
		return data.map((note, index) => {
			...
			return {
				...
				dateCreated: { value: this._date.transform(note.dateCreated, 'MMM dd, yyyy'), editable: false, hidden: false },
	...
	private subscribeToDeleteNoteFromTable(): void {
		...
		this._release.onDeleteNoteFromDataTable.pipe(takeUntil(this._unsubscribe))
			.subscribe(
				({ releaseNoteId }) => {

					this._release.deleteNote(releaseNoteId).pipe(takeUntil(this._unsubscribe))
						.subscribe(
							() => {
	...
	private subscribeToEditNoteFromTable(): void {
		...
		this._release.onEditNoteFromDataTable.pipe(takeUntil(this._unsubscribe))
			.subscribe(
				({ releaseNoteId }) => {
					...
					dialog.afterClosed().subscribe(
						(response: boolean | API_RELEASE_NOTE) => {
